...
def cortex_chat(user_question: str, context: str) -> str:
    prompt = (
        ...
        "데이터에 기반한 구체적이고 실행 가능한 답변을 한국어로 제공하세요. "
        "숫자와 비율을 인용하며 근거를 밝히세요. 모르는 내용은 모른다고 하세요.\n\n"
        f"[대시보드 데이터 컨텍스트]\n{context}\n\n[질문]\n{user_question}"
    )
        ...
        with st.expander("🤖 Cortex AI: 사업 현황 종합 분석"):
            ...
            if _tab1_ai_key in st.session_state:
                ...
                tab1_prompt = (
                    ...
                    "1) 완료율 변동의 근본 원인 3가지\n"
                    "2) 각 원인에 대한 구체적 개선 방향\n"
                    "3) 가장 시급하게 조치해야 할 1가지\n"
                    "를 한국어로 간결하게 답변하세요.\n\n" + tab1_ai_ctx
                )
            ...
            if cortex_ready:
                with st.expander("🤖 Cortex AI: 렌탈 상품 완료율 원인 분석"):
                    ...
                    if _diag1_key in st.session_state:
                        ...
                        diag1_prompt = (
                            "다음 렌탈 상품들의 완료율이 낮은 근본 원인을 추론하세요. "
                            "상품별 특성(설치 난이도, 계절성, 가격대, 고객층)을 고려하여 "
                            "각 상품별 원인과 개선 전략을 구체적으로 제시하세요.\n\n" + product_ctx
                        )
                        with st.spinner("Cortex AI 분석 중..."):
                ...
                with st.expander("🤖 Cortex AI: 우선 점검 구간 심층 분석"):
                    ...
                    if _diag2_key in st.session_state:
                        ...
                        diag2_prompt = (
                            ...
                            "각 구간의 문제 원인을 지역 특성(인구밀도, 접근성, 경쟁 강도)과 "
                            "상품 특성(설치 복잡도, 계절 수요)을 고려하여 분석하고, "
                            "우선 조치 사항을 제시하세요.\n\n" + diag2_ctx
                        )
                        with st.spinner("Cortex AI 분석 중..."):
            ...
            if _sc_key in st.session_state:
                ...
                with st.spinner("Cortex AI가 대응 전략을 생성하고 있습니다..."):
                    scenario_prompt = (
                        ...
                        f"{strategy_context}\n\n"
                        f"아래 내용을 작성하세요:\n"
                        f"1) 즉시 실행할 대응 액션 3가지 (구체적, 숫자/기간 포함)\n"
                        f"2) 가장 영향 받는 구간과 예상 규모\n"
                        f"3) 리스크와 완화 방안"
                    )
